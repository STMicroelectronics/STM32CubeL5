...
AUDIO_ErrorTypeDef PLAYER_Start(char * file_name, uint32_t volume)
{
  ...
  if(file_name != NULL)  {
    ...
    if (PLAYER_StreamInit(AUDIO_FREQUENCY_48K, AUDIO_OUT_HEADPHONE, AudioFormatData.BitPerSample, AudioFormatData.NbrChannels, volume) != 0)
    {
      return AUDIO_ERROR_CODEC;
    }
    ...
    if ( error == AUDIO_ERROR_NONE)
    {
      if(BufferCtl.fptr != 0)
      {
        /*DMA channel from output double buffer to codec in Circular mode launch*/
