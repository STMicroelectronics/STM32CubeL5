 ...
 */
template <uint16_t capacity, uint32_t bytes_pr_pixel>
...
public:
    ...
    virtual uint16_t allocateBlock(const uint16_t x, const uint16_t y, const uint16_t width, const uint16_t height, uint8_t** block)
    {
        ...
        const int32_t lines = capacity / stride;
 ...
 */
template <uint32_t CAPACITY, int32_t BLOCKS, uint32_t bytes_pr_pixel>
...
private:
    ...
    uint32_t memory[BLOCKS][CAPACITY / 4];
