...
static void ComputeImplementationId(uint8_t *hash_result)
{
  ...
  bootutil_sha256_update(&sha256_ctx,
                         &REGION_NAME(Image$$, ER_CODE, $$Base),
                         (uint32_t)&REGION_NAME(Image$$, ER_CODE, $$Limit) -
                         (uint32_t)&REGION_NAME(Image$$, ER_CODE, $$Base));
  BOOT_LOG_INF("Code %x %x", (uint32_t)&REGION_NAME(Image$$, ER_CODE, $$Base)
               , (uint32_t)&REGION_NAME(Image$$, ER_CODE, $$Limit));
