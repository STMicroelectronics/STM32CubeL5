...
static inline bool Write_Header(void)
{
...
#ifdef TFM_HUK_RANDOM_GENERATION
  __IO NVCNT_DATA_TYPE huk[HUK_SIZE / sizeof(NVCNT_DATA_TYPE)];
  NVCNT_ID_TYPE huk_random_id = NVCNT_ID_HUK;
  size_t len = sizeof(huk);
#else
  ...
  mbedtls_hardware_poll(NULL, (uint8_t *)&huk, sizeof(huk), &len);
  BOOT_LOG_INF("BL2 HUK generated = %x%x..%x%x", huk[0], huk[1], huk[6], huk[7]);
  if (len != sizeof(huk))
  {
    BOOT_LOG_ERR("BL2 HUK generation Failed !!");
    Error_Handler();
  }
#else
...
#ifdef TFM_HUK_RANDOM_GENERATION
/**
  * @brief  This function writes the HUK in BL2 DATA SHARED.
  * @param : huk  pointer to uint8_t array
  * @param : huk_size array size
  * @retval boolean return true on success
  */
static bool write_huk(const uint8_t *huk, uint32_t huk_size)
{
...
{
#ifdef TFM_HUK_RANDOM_GENERATION
  /* Set to __IO to force value cleaning */
  __IO NVCNT_ELEMENT_TYPE addressvalue = 0U;
  __IO NVCNT_DATA_TYPE huk[HUK_SIZE / sizeof(NVCNT_DATA_TYPE)];
#else
  NVCNT_ELEMENT_TYPE addressvalue;
  __IO NVCNT_DATA_TYPE huk[1];
  ...
  if (ret)
  {
    ret = write_huk((const uint8_t *)huk, sizeof(huk));
  }
  memset((void *)huk, 0, sizeof(huk));
  addressvalue  = 0;
#else
