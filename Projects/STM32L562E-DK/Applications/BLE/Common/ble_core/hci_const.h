...
typedef __PACKED_STRUCT _hci_uart_pckt{
  uint8_t  type;
  uint8_t  data[VARIABLE_SIZE];
} PACKED hci_uart_pckt;
#define HCI_HDR_SIZE           1

typedef __PACKED_STRUCT _hci_command_hdr{
  uint16_t  opcode;    /* OCF & OGF */
  uint8_t    plen;
} PACKED hci_command_hdr;
#define HCI_COMMAND_HDR_SIZE   3

typedef __PACKED_STRUCT _hci_event_pckt{
  uint8_t    evt;
  uint8_t    plen;
  uint8_t      data[VARIABLE_SIZE];
...
typedef __PACKED_STRUCT _hci_acl_hdr{
...
typedef __PACKED_STRUCT _disconnect_cp{
  uint16_t  handle;
  uint8_t    reason;
...
typedef __PACKED_STRUCT _read_transmit_power_level_cp{
  uint16_t  handle;
  uint8_t    type;
} PACKED read_transmit_power_level_cp;
#define READ_TRANSMIT_POWER_LEVEL_CP_SIZE  3

typedef __PACKED_STRUCT _read_transmit_power_level_rp{
  uint8_t    status;
  uint16_t  handle;
  int8_t    level;
...
typedef __PACKED_STRUCT _read_local_version_rp{
  uint8_t    status;
  uint8_t    hci_version;
...
typedef __PACKED_STRUCT _read_bd_addr_rp{
  uint8_t    status;
  tBDAddr    bdaddr;
...
typedef __PACKED_STRUCT _read_rssi_cp{
  uint16_t  handle;
} PACKED read_rssi_cp;

#define READ_RSSI_CP_SIZE 2
typedef __PACKED_STRUCT _read_rssi_rp{
  uint8_t    status;
  uint16_t  handle;
  int8_t    rssi;
} PACKED read_rssi_rp;
...
typedef __PACKED_STRUCT _le_set_event_mask_cp{
  uint8_t    mask[8];
} PACKED le_set_event_mask_cp;
#define LE_SET_EVENT_MASK_CP_SIZE 8

#define OCF_LE_READ_BUFFER_SIZE      0x0002
typedef __PACKED_STRUCT _le_read_buffer_size_rp{
  uint8_t    status;
  uint16_t  pkt_len;
  uint8_t    max_pkt;
...
typedef __PACKED_STRUCT _le_read_local_supported_features_rp{
  uint8_t    status;
  uint8_t    features[8];
...
typedef __PACKED_STRUCT _le_read_adv_channel_tx_power_rp{
  uint8_t    status;
  int8_t    level;
...
typedef __PACKED_STRUCT _le_set_adv_data_cp{
  uint8_t    length;
  uint8_t    data[31];
...
typedef __PACKED_STRUCT _le_set_scan_response_data_cp{
  uint8_t    length;
  uint8_t    data[31];
...
typedef __PACKED_STRUCT _le_set_advertise_enable_cp{
  uint8_t    enable;
} PACKED le_set_advertise_enable_cp;
#define LE_SET_ADVERTISE_ENABLE_CP_SIZE 1

#define OCF_LE_SET_SCAN_PARAMETERS    0x000B
typedef __PACKED_STRUCT _le_set_scan_parameters_cp{
  uint8_t    type;
  uint16_t  interval;
  uint16_t  window;
  uint8_t    own_bdaddr_type;
...
typedef __PACKED_STRUCT _le_set_scan_enable_cp{
  uint8_t    enable;
  uint8_t    filter_dup;
...
typedef __PACKED_STRUCT _le_create_connection_cp{
  uint16_t  interval;
  uint16_t  window;
...
typedef __PACKED_STRUCT _le_read_white_list_size_rp{
  uint8_t    status;
  uint8_t    size;
...
typedef __PACKED_STRUCT _le_add_device_to_white_list_cp{
  uint8_t    bdaddr_type;
  tBDAddr   bdaddr;
...
typedef __PACKED_STRUCT _le_remove_device_from_white_list_cp{
  uint8_t    bdaddr_type;
  tBDAddr   bdaddr;
...
typedef __PACKED_STRUCT _le_connection_update_cp{
  uint16_t  handle;
  uint16_t  min_interval;
  uint16_t  max_interval;
  uint16_t  latency;
...
typedef __PACKED_STRUCT _le_set_host_channel_classification_cp{
  uint8_t    map[5];
...
typedef __PACKED_STRUCT _le_read_channel_map_cp{
  uint16_t  handle;
} PACKED le_read_channel_map_cp;
#define LE_READ_CHANNEL_MAP_CP_SIZE 2

typedef __PACKED_STRUCT _le_read_channel_map_rp{
  uint8_t    status;
  uint16_t  handle;
  uint8_t    map[5];
...
typedef __PACKED_STRUCT _le_rand_rp{
  uint8_t  status;
  uint8_t  random[8];
} PACKED le_rand_rp;
...
typedef __PACKED_STRUCT _le_start_encryption_cp{
  uint16_t  handle;
  uint8_t      random[8];
...
typedef __PACKED_STRUCT _le_ltk_reply_cp{
  uint16_t  handle;
  uint8_t    key[16];
} PACKED le_ltk_reply_cp;
#define LE_LTK_REPLY_CP_SIZE 18

typedef __PACKED_STRUCT _le_ltk_reply_rp{
  uint8_t    status;
  uint16_t  handle;
...
typedef __PACKED_STRUCT _le_ltk_neg_reply_cp{
  uint16_t  handle;
} PACKED le_ltk_neg_reply_cp;
#define LE_LTK_NEG_REPLY_CP_SIZE 2

typedef __PACKED_STRUCT _le_ltk_neg_reply_rp{
  uint8_t    status;
  uint16_t  handle;
...
typedef __PACKED_STRUCT _le_read_supported_states_rp{
  uint8_t    status;
  uint8_t      states[8];
...
typedef __PACKED_STRUCT _le_receiver_test_cp{
  uint8_t    frequency;
} PACKED le_receiver_test_cp;
#define LE_RECEIVER_TEST_CP_SIZE 1

#define OCF_LE_TRANSMITTER_TEST      0x001E
typedef __PACKED_STRUCT _le_transmitter_test_cp{
  uint8_t    frequency;
  uint8_t    length;
  uint8_t    payload;
...
typedef __PACKED_STRUCT _le_test_end_rp{
  uint8_t    status;
  uint16_t  num_pkts;
...
typedef __PACKED_STRUCT _evt_conn_complete{
  uint8_t    status;
  uint16_t  handle;
  tBDAddr   bdaddr;
  uint8_t    link_type;
...
typedef __PACKED_STRUCT _evt_disconn_complete{
  uint8_t    status;
  uint16_t  handle;
  uint8_t    reason;
...
typedef __PACKED_STRUCT _evt_encrypt_change{
  uint8_t    status;
  uint16_t  handle;
  uint8_t    encrypt;
...
typedef __PACKED_STRUCT _evt_cmd_complete{
  uint8_t    ncmd;
  uint16_t  opcode;
#ifndef ADDED_BY_MCD
...
typedef __PACKED_STRUCT _evt_cmd_status{
  uint8_t    status;
  uint8_t    ncmd;
  uint16_t  opcode;
...
typedef __PACKED_STRUCT _evt_hardware_error{
  uint8_t    code;
} PACKED evt_hardware_error;
#define EVT_HARDWARE_ERROR_SIZE 1

#define EVT_NUM_COMP_PKTS    0x13
typedef __PACKED_STRUCT _evt_num_comp_pkts{
  uint8_t    num_hndl;
...
typedef __PACKED_STRUCT _evt_num_comp_pkts_param{
  uint16_t    hndl;
...
typedef __PACKED_STRUCT _evt_data_buffer_overflow{
  uint8_t    link_type;
} PACKED evt_data_buffer_overflow;
#define EVT_DATA_BUFFER_OVERFLOW_SIZE 1

#define EVT_ENCRYPTION_KEY_REFRESH_COMPLETE  0x30
typedef __PACKED_STRUCT _evt_encryption_key_refresh_complete{
  uint8_t    status;
  uint16_t  handle;
...
typedef __PACKED_STRUCT _evt_le_meta_event{
  uint8_t    subevent;
  uint8_t    data[VARIABLE_SIZE];
...
typedef __PACKED_STRUCT _evt_le_connection_complete{
  uint8_t    status;
  uint16_t  handle;
  uint8_t    role;
  uint8_t    peer_bdaddr_type;
...
typedef __PACKED_STRUCT _le_advertising_info{
  uint8_t               num_reports;
  uint8_t    evt_type;
  uint8_t    bdaddr_type;
  tBDAddr      bdaddr;
...
typedef __PACKED_STRUCT _evt_le_connection_update_complete{
  uint8_t    status;
  uint16_t  handle;
  uint16_t  interval;
  uint16_t  latency;
...
typedef __PACKED_STRUCT _evt_le_read_remote_used_features_complete{
  uint8_t    status;
  uint16_t  handle;
  uint8_t    features[8];
...
typedef __PACKED_STRUCT _evt_le_long_term_key_request{
  uint16_t  handle;
  uint8_t      random[8];
  uint16_t  ediv;
