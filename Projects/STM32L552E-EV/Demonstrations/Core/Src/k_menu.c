  ...
  case TYPE_ICON :
    {
      ...
      if(kStorage_OpenFileDrawBMP(psCurrentMenu.psItems[*sel].x+1, psCurrentMenu.psItems[*sel].y+1, (uint8_t *)psCurrentMenu.psItems[*sel].pIconSelectedPath) != KSTORAGE_NOERROR)
      {
        kWindow_Error("Resource error\n");
        while(1);
      }
      ...
      do{
        ...
        if(joyState != JOY_NONE)
        {
          ...
          if(kStorage_OpenFileDrawBMP(psCurrentMenu.psItems[*sel].x+1, psCurrentMenu.psItems[*sel].y+1, (uint8_t *)psCurrentMenu.psItems[*sel].pIconPath) != KSTORAGE_NOERROR)
          {
            kWindow_Error("Resource error\n");
            while(1);
          }
    ...
    case KMENU_ICON :
      {
        for(index = 0; index < psCurrentMenu.nItems; index++)
        {
          ...
          {
            if(kStorage_OpenFileDrawBMP(psCurrentMenu.psItems[index].x+1, psCurrentMenu.psItems[index].y+1, (uint8_t *)psCurrentMenu.psItems[index].pIconPath) != KSTORAGE_NOERROR)
            {
              kWindow_Error("Resource error\n");
              while(1);
            }
