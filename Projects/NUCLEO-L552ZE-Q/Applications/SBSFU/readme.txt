...
@par Directory contents
   ...
   - Linker      Linker files definition shared between SBSFU_Boot and SBSFU_Appli
...
1. Build
   ...
   2- Build SBSFU_Appli Secure project.
      This step creates the SBSFU Secure binary:
      ...
      It also produces the SBSFU Secure Application signed  binary in SBSFU_Appli\Binary\sbsfu_s_sign.bin.

   3- Build SBSFU_Appli Non Secure project.
      This step creates the SBSFU Non Secure binary:
        ...
        MDK-ARM:      SBSFU_Appli\MDK-ARM\NUCLEO-L552ZE-Q_SBSFU_N\Exe\Project.bin
      It also produced the SBSFU Non Secure Application signed binary in SBSFU_Appli\Binary\sbsfu_ns_sign.bin.
...
2. Initialize the device
   ...
   shot:
     STM32CubeIDE: SBSFU_Boot\STM32CubeIDE\regression.bat or regression.sh
...
4. Program the SBSFU into internal flash

   Depending on your toolchain, execute script SBSFU_UPDATE (relying on
   STM32CubeProgrammer CLI tool) to easily program the SBSFU Boot and Application into device internal
   flash in one shot:
     STM32CubeIDE: SBSFU_Boot\STM32CubeIDE\SBSFU_UPDATE.bat or SBSFU_UPDATE.sh
...
6. Unlock the board
   ...
   To exit from locked state (intrusion), remove the jumper JP5 on the board,
   then put it back in place. Check the log information:
     - SBSFU_Boot starts
     - Then it checks the static protections
     - Then it jumps to the SBSFU_Appli application, displaying the User Application
       Main menu.
...
7. Use User App
   ...
   1. Press 1 to enter New Fw Download menu
      ...
      It is possible to download a new SBSFU Secure image or SBSFU Non Secure image or both.
      - Download Secure Image (#1): Download Secure signed image (ex: SBSFU_Appli\binary\sbsfu_s_sign.bin)
      - Download NonSecure Image (#2): Download Non Secure signed image (ex: SBSFU_Appli\binary\sbsfu_ns_sign.bin)
      ...
      installed and executed, by SBSFU_Boot.
   ...
   3. Press 3 to toggle Secure LED
